...
Bootstrap dell'ambiente la prima volta: `powershell -ExecutionPolicy Bypass -File scripts\setup-tex.ps1` (Windows) oppure `sh scripts/setup-tex.sh` (Unix). Compilazione: `powershell -ExecutionPolicy Bypass -File scripts\build.ps1` / `sh scripts/build.sh`, che invocano `latexmk` sull'engine pdflatex fissato in `.latexmkrc`. Pulizia degli ausiliari: `build.* -Clean` (lascia il PDF) o `-CleanAll` (rimuove anche il PDF). Gli ausiliari LaTeX e il PDF sono ignorati da git.
